...
%%
function [iNO_end, iEL_end] = PP_model_LowerInner(fileID, iNO, iEL, Lower_arc_num, Lower_angle, Lower_angle_origin, Lower_CoC, Lower_R, Lower_Sp, Stadium_R, Stadium_arc_num, Stadium_angle, ~)
%% NODE
...
Lower_Z_R = zeros(Lower_Z_num2+1,1); % 与Z标高对应的stadium_R方向的半径，圆心同stadium_R
...
fprintf(fileID,'; 下层网格内 节点\n');
iNO_Lower_init = iNO; % 备份
for i = 1:Lower_Z_num2+1 % 此处与上不同
...
fprintf(fileID,'; 下层网格内 杆件\n');
ELE_iPRO = 2;
iNO = iNO_Lower_init; % 初始化iNO
for i = 1:Lower_Z_num2 % 此处与上不同 类似柱子，循环数为层数-1
